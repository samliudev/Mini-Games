...
class Image:
    def __init__(self, x_pixels=0, y_pixels=0, num_channels=0, filename=''):
        # you need to input either filename OR xpixels, ypixels, and numchannels
        ...
        if x_pixels and y_pixels and num_channels:
            ...
            self.array = np.zeros((x_pixels, y_pixels, num_channels)) # 3d matrix initialized to 0
        elif filename:
    ...
    def read_image(self, filename, gamma=2.2): # gamma correction is used to encode/decode luminance in still images 
        '''
