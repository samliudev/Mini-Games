...
def adjust_brightness(image, factor):
    # when we adjust brightness, we just want to scale each value by some amount
...
def blur(image, kernel_size):
    # kernel size is the amount of pixels to take into account when applying the blur
    # (ie kernel_size = 3 would be neighbros to the left/right, top/bottom, and diagonals)
    ...
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                # naive implementation of iterating through each neighbor and summing
                # memoization would be faster
                total = 0
                ...
                new_im.array[x, y, c] = total / (kernel_size ** 2) # average = sum of pixels divided by # of pixels       
...
def apply_kernel(image, kernel):
    ...
    # we'll assume the kernel is square
    # for example the sobel x kernel (detecting horizontal ridges) is as follows:
    # [1 0 -1]
    # [2 0 -2]
    # [1 0 -1]
    x_pixels, y_pixels, num_channels = image.array.shape
...
def combine_images(image1, image2):
    ...
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                # sqrt of the sum of both images squared
...
if __name__ == '__main__':
    ...
    # # darken
    # darkened_im = adjust_brightness(lake, .3)
