...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # it to an integeter, and if it's not, then we say its invalid
            # if that spot is not available on the board, we also sasy its invalid
            try:
                val = int(square)
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if state.current_winner == other_player:
            ...
            # take number of empty squares + 1, * 1 if computer player, * -1 if user player
        ...
        if player == max_player:
            best = {'position': None, 'score': -math.inf} # trying to get max score
        else: # player == other player
            best = {'position': None, 'score': math.inf} # trying to get min score

        for possible_move in state.available_moves():
            # 1: make a move, try that spot
            state.make_move(possible_move, player)

            # 2: recurse using minimax to stimulate a game after making that move
